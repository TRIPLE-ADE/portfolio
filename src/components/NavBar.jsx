...
function NavBar({ handleClick, darkMode }){
    ...
    <nav className={`flex items-center`}>
        ...
        <div className='flex ml-auto items-center gap-7'>
            ...
            <input type="checkbox" name="mode-switch" id="mode-switch" className='w-0 h-0'/>
            <label htmlFor="mode-switch" className='bg-deep-blue dark:bg-gray w-10 h-5 block relative shadow-inner rounded-xl dark:before:bg-deep-blue' onClick={handleClick}>
                <img src={darkMode ? moon1 : sun} alt="" className={`absolute ${darkMode ? 'right-1' : 'left-1'} w-3 top-0 bottom-0 m-auto`} />
            </label>
        </div>
    </nav>
)}
